...
class BeritaController extends Controller
{
    ...
    public function apiUpdate(Request $request, $id)
    {
        try {
            ...
            $validated = $request->validate([
                'nama_berita' => 'sometimes|required|string',
                'kode_divisi' => 'sometimes|required|string',
                'prioritas_id' => 'sometimes|required|integer',
            ]);

            // 🔄 Update hanya field yang dikirim
            if ($request->has('nama_berita')) $berita->nama_berita = $request->nama_berita;
            if ($request->has('kode_divisi')) $berita->kode_divisi = $request->kode_divisi;
            if ($request->has('prioritas_id')) $berita->prioritas_id = $request->prioritas_id;

            $berita->facebook = $request->facebook ?? $berita->facebook;
            $berita->website = $request->website ?? $berita->website;
            $berita->instagram = $request->instagram ?? $berita->instagram;
            $berita->twitter = $request->twitter ?? $berita->twitter;
            $berita->tiktok = $request->tiktok ?? $berita->tiktok;
            $berita->sippn = $request->sippn ?? $berita->sippn;
            $berita->youtube = $request->youtube ?? $berita->youtube;

            // 🧩 Tangani JSON array agar tidak double encode
            if ($request->has('media_lokal')) {
                $berita->media_lokal = $this->normalizeMediaInput($request->media_lokal);
            }

            if ($request->has('media_nasional')) {
                $berita->media_nasional = $this->normalizeMediaInput($request->media_nasional);
            }
